...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("mail_agent.log"),
...
def process_emails(config):
    ...
    try:
        ...
        for email in emails:
            ...
            for label in labels:
                label_config = next((l for l in config['labels'] if l['name'] == label), None)
...
def main():
    ...
    try:
        ...
        schedule.every(config['email']['check_frequency']).seconds.do(process_emails, config)
